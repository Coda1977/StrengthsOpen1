    ...
    try {
      ...
      const verifyEnvironmentVariables = () => {
        ...
        if (process.env.CLERK_SECRET_KEY) {
          console.log('[ENV] CLERK_SECRET_KEY configured');
        ...
        if (process.env.CLERK_PUBLISHABLE_KEY) {
          console.log('[ENV] CLERK_PUBLISHABLE_KEY configured');
          if (!process.env.CLERK_PUBLISHABLE_KEY.startsWith('pk_')) {
            warnings.push('CLERK_PUBLISHABLE_KEY should start with pk_');
          }
        }
      ...
      // For Vercel, we don't use Vite dev server in production
      ...
      // Vercel serverless functions don't use listen()
